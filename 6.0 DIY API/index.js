...
app.patch("/jokes/:id",(req,res)=>{
  ...
  const PjokeIndex= jokes.findIndex((joke)=>joke.id === id);
  jokes[PjokeIndex]=PatchJoke; 
  console.log(jokes[PjokeIndex]);
  res.json(PatchJoke);
...
app.delete("/jokes/:id",(req,res)=>{
  ...
  const searchIndex=jokes.findIndex((joke)=>joke.id === id);
    if(searchIndex>-1){
      ...
      res
      .status(404)
      .json({error:`Joke with ${id} not found. No jokes were found.`})
    }
...
app.delete("/all",(req,res)=>{
  ...
  if(userKey === masterKey){
    ...
    res
    .status(404)
    .json({error:`Error you are not authorized to perform that action`})
  )
})
